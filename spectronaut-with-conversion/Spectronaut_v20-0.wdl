...
workflow panoply_spectronaut {
  ...
  if (do_conversion) {
    call convert_htrms {
      input: 
        files_folder=select_first([files_folder]) 
    }
  }
  ...
  if (do_search) {
    call spectronaut {
      input:
        ...
        files_folder=select_first([convert_htrms.htrms_dir, files_folder])
    }
  }
}
...
task convert_htrms {
  ...
  command <<<
    ...
    -i ~{files_folder} \
    -o htrms_converted \
    ~{if defined(convert_scheme) then " -s " + convert_scheme else ""}
