...
workflow panoply_spectronaut_parallel {
    input {
        ...
        # Flexible VM scaling configuration
        Int n_vms = 1                        # Maximum number of VMs to use (1-5 based on token budget)
        Int parallel_jobs_per_vm = 5         # Number of concurrent searches per VM (3-6 recommended)

        # VM resources (per VM)
        Int vm_cpu = 128                     # CPU cores per VM (AMD Rome max)
        Int vm_memory_gb = 896               # RAM per VM in GB (AMD Rome max)
        Int vm_disk_gb_base = 500            # Base disk space per VM

        # Dynamic disk sizing
        Int file_size_multiplier = 3
    }

    # List files from directory if input_file_list not provided
    ...
    # Smart VM allocation logic
    Int total_files = length(files_to_search)
    Int actual_vms = if (total_files < parallel_jobs_per_vm) then 1
                     else if (n_vms * parallel_jobs_per_vm >= total_files) then ceil(total_files / parallel_jobs_per_vm)
                     else n_vms

    # Calculate per-thread resource allocation
    Int cpu_per_thread = vm_cpu / parallel_jobs_per_vm
    Int memory_per_thread = vm_memory_gb / parallel_jobs_per_vm

    # Calculate disk per VM
    Float total_files_size_gb = size(files_to_search, "GB")
    Int vm_disk_gb = vm_disk_gb_base + ceil(total_files_size_gb * file_size_multiplier * parallel_jobs_per_vm / total_files)

    # Step 2: Run Spectronaut search on multiple VMs with parallel execution per VM
    if (do_search) {
        # Create batches for distribution across VMs
        call make_batches {
            input:
                files = files_to_search,
                n_batches = actual_vms
        }

        # Launch VMs; each VM processes its batch in parallel
        scatter (batch_list in make_batches.batch_lists) {
            call spectronaut_search_parallel_batch {
                input:
                    ...
                    parallel_jobs = parallel_jobs_per_vm,
                    cpu_per_job = cpu_per_thread,
                    memory_gb_per_job = memory_per_thread,
                    cpu = vm_cpu,
        ...
        call combine_sne_files {
            input:
                sne_files = flatten(spectronaut_search_parallel_batch.sne_outputs),
                experiment_name = experiment_name,
    ...
    output {
        ...
        Int vms_used = actual_vms
        Int total_parallelism = actual_vms * parallel_jobs_per_vm
    }
    ...
    parameter_meta {
        ...
        n_vms: "Maximum number of VMs to use (1-5 tokens). Workflow automatically uses fewer if optimal."
        parallel_jobs_per_vm: "Number of concurrent Spectronaut searches per VM (3-6 recommended). Lower values = more resources per search."
        vm_cpu: "CPU cores per VM (default 128 = AMD Rome max)"
        vm_memory_gb: "RAM per VM in GB (default 896 = AMD Rome max)"
    ...
    meta {
        ...
        description: "Flexible parallel Spectronaut workflow with HTRMS conversion. Allows users to configure token usage (1-5 VMs) and parallelism per VM (3-6 concurrent searches). Smart allocation automatically optimizes VM usage based on file count. Each VM runs multiple Spectronaut searches concurrently using GNU Parallel. Supports two input methods: (1) Directory containing files, or (2) Text file listing file paths."
    }
}
...
task convert_htrms_single {
    ...
    command <<<
        ...
        # Searches for the first .htrms file in the output directory and store its path 
        converted_file=$(find htrms_converted -type f -name "*.htrms" | head -n 1)

        # Validate that a converted file was found 
...
# Task 3: Create N batch lists from a list of files
# AWK-based approach: reads every line in the file list once and decide which batch file to write based on modulo operation 
# AWK is more efficient and flexible than basic split approach (using native GNU `split` command to divide the file into N parts - limit to equal sizes only)
task make_batches {
    input {
        Array[File] files
        Int n_batches
    ...
    command <<<
        ...
        # Write all files (one per line) safely, then distribute round-robin
        # Materialize the input list into a plain text file --> all_files.txt
...
EOF
        ...
        if [ "${total}" -eq 0 ]; then
          ...
          # Use AWK for efficient single-pass distribution
...
# Task 4: Run Spectronaut search on batch list with parallel execution
task spectronaut_search_parallel_batch {
    input {
        File list_file
        String experiment_name
    ...
    command <<<
        ...
        echo "=== Spectronaut parallel batch search (broadcptacdev/panoply_spectronaut:v20.0) ===" >&2
        ...
        # Read files from batch list
        mapfile -t files < ~{list_file}
        file_count=${#files[@]}
        echo "Files in this batch: ${file_count}" >&2
        ...
        # Get cromwell root for absolute paths
        cromwell_root=$(pwd)
        echo "Cromwell root: ${cromwell_root}" >&2

        # Install GNU Parallel if not present
        if ! command -v parallel &> /dev/null; then
            echo "GNU Parallel not found, installing..." >&2
            export DEBIAN_FRONTEND=noninteractive
            apt-get update -qq && apt-get install -y -qq parallel > /dev/null 2>&1
            echo "GNU Parallel installed successfully" >&2
        else
            echo "GNU Parallel already available" >&2
        fi
        echo "" >&2

        # Import enzyme database once per VM if provided
        ...
        search_one_file() {
            ...
            if [ ! -e "$input_file" ]; then
                echo "[Job ${job_id}] WARNING: Skipping missing file: $input_file" >&2
                return 1
            fi
            ...
            local file_exp_name="~{experiment_name}_${base_name}"
            ...
            local sne_file
            sne_file=$(find "${work_dir}/spectronaut_out" -type f -name "*.sne" | head -n 1)

            if [ -z "$sne_file" ]; then
                echo "[Job ${job_id}] ERROR: No .sne file found for ${input_basename}" >&2
                echo "[Job ${job_id}] Contents of spectronaut_out:" >&2
                find "${work_dir}/spectronaut_out" -type f >&2
                rm -rf "${work_dir}" "${sn_temp}"
                return 1
            fi
            ...
            local sne_size
            sne_size=$(du -h "${cromwell_root}/outputs/${out_name}" | cut -f1)
        ...
        parallel \
            ...
            --halt soon,fail=20% \
            --timeout 0 \
            search_one_file {} {#} < batch_files.txt

        parallel_exit=$?

        echo "" >&2
        echo "=== Parallel execution complete for this batch ===" >&2
        ...
        if [ "${sne_count}" -gt 0 ]; then
            echo "Successfully created SNE files:" >&2
            ls -lh outputs/ >&2
            echo "" >&2
        else
            echo "WARNING: No SNE files were created in this batch" >&2
            echo "All jobs in this batch failed. Check individual log files in logs/ directory" >&2
        fi
    ...
    meta {
        author: "C. Lian"
        email: "proteogenomics@broadinstitute.org"
        description: "Processes a batch of files with parallel Spectronaut searches using GNU Parallel. Part of flexible multi-VM workflow."
    }
}

# Task 5: Combine all SNE files into one
